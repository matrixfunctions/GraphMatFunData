...
@inline function mono_m13(A)
    return mono_m13!(copy(A))
end
...
@inline function mono_m13!(A)
    ...
    value_one=ValueOne()
    # Computation order: B2 B3 B4 B5 B6 B7 B8 B9 B10 B11 B12 B13 B14 T2k16
    ...
    mul!(memslots[12],memslots[11],memslots[1])
    # Computing B13 with operation: mult
    mul!(memslots[13],memslots[12],memslots[1])
    # Computing B14 with operation: mult
    mul!(memslots[14],memslots[13],memslots[1])
    # Computing T2k16 = x*I+x*A+x*B2+x*B3+x*B4+x*B5+x*B6+x*B7+x*B8+x*B9+x*B10+x*B11+x*B12+x*B13+x*B14
    coeff1=1.0
    coeff2=1.0
    coeff3=0.5
    ...
    coeff13=2.08767569878681e-9
    coeff14=1.6059043836821613e-10
    coeff15=1.1470745597729725e-11
    # Smart lincomb recycle A
    memslots[1] .= coeff2.*memslots[1] .+ coeff3.*memslots[2] .+ coeff4.*memslots[3] .+ coeff5.*memslots[4] .+ coeff6.*memslots[5] .+ coeff7.*memslots[6] .+ coeff8.*memslots[7] .+ coeff9.*memslots[8] .+ coeff10.*memslots[9] .+ coeff11.*memslots[10] .+ coeff12.*memslots[11] .+ coeff13.*memslots[12] .+ coeff14.*memslots[13] .+ coeff15.*memslots[14]
    ...
    # Deallocating B12 in slot 12
    # Deallocating B13 in slot 13
    # Deallocating B14 in slot 14
    return memslots[1] # Returning T2k16
end
