...
@inline function sid_m13(A)
    return sid_m13!(copy(A))
end
...
@inline function sid_m13!(A)
    ...
    value_one=ValueOne()
    # Computation order: Bb2 Ba2 B2 Ba3 B3 Ba4 B4 Ba5 B5 Bb6 B6 Ba7 Bb7 B7 Ba8 Bb8_7 B8 Ba9 Bb9 B9 B10 B11 B12 B13 B14 T2k16
    ...
    memslots[9] .= coeff1.*memslots[1] .+ coeff2.*memslots[4] .+ coeff3.*memslots[3] .+ coeff4.*memslots[5] .+ coeff5.*memslots[6] .+ coeff6.*memslots[7] .+ coeff7.*memslots[9]
    # Computing Bb8_7 = x*A+x*B2+x*B3+x*B4+x*B5+x*B6
    coeff1=-4.12224409740275e-6
    ...
    mul!(memslots[2],memslots[9],memslots[7])
    ...
    coeff7=1.0
    memslots[7] .= coeff2.*memslots[1] .+ coeff3.*memslots[4] .+ coeff4.*memslots[3] .+ coeff5.*memslots[5] .+ coeff6.*memslots[6] .+ coeff7.*memslots[2]
    ...
    memslots[1] .= coeff2.*memslots[1] .+ coeff3.*memslots[4] .+ coeff4.*memslots[3] .+ coeff5.*memslots[5] .+ coeff6.*memslots[6] .+ coeff7.*memslots[2]
    mul!(memslots[1],true,I*coeff1,true,true)
    # Deallocating B3 in slot 3
    # Deallocating B4 in slot 5
    # Deallocating B5 in slot 6
    # Deallocating B8 in slot 2
    # Computing B9 with operation: mult
    mul!(memslots[2],memslots[7],memslots[1])
    # Deallocating Ba9 in slot 7
    # Deallocating Bb9 in slot 1
    # Computing B10 with operation: mult
    mul!(memslots[1],memslots[2],memslots[2])
    ...
    mul!(memslots[1],memslots[2],memslots[2])
    # Deallocating B13 in slot 2
    # Computing T2k16 = x*B2+x*B14
    coeff1=0.0
    coeff2=1.0
    # Smart lincomb recycle B2
    memslots[4] .= coeff1.*memslots[4] .+ coeff2.*memslots[1]
    # Deallocating B14 in slot 1
    return memslots[4] # Returning T2k16
end
