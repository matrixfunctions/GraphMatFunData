...
@inline function exp_ps_m8(A)
    T=promote_type(eltype(A),Float64)
    A_copy=similar(A,T); A_copy .= A;
    return exp_ps_m8!(A_copy)
end
...
@inline function exp_ps_m8!(A)
    ...
    value_one=ValueOne()
    # Computation order: A2 A3 A4 A5 P4 C3 P3 C2 P2 C1 P1 C0 P0
    ...
    # Computing P4 = x*I+x*A+x*A2+x*A3+x*A4+x*A5
    coeff1=4.110317623312165e-19
    coeff2=1.9572941063391263e-20
    coeff3=8.896791392450574e-22
    coeff4=3.868170170630684e-23
    coeff5=1.6117375710961184e-24
    coeff6=6.446950284384474e-26
    memslots6 .= coeff2.*memslots1 .+ coeff3.*memslots2 .+ coeff4.*memslots3 .+ coeff5.*memslots4 .+ coeff6.*memslots5
    mul!(memslots6,true,I*coeff1,true,true)
    # Computing C3 with operation: mult
    mul!(memslots7,memslots6,memslots5)
    # Deallocating P4 in slot 6
    # Computing P3 = x*I+x*A+x*A2+x*A3+x*A4+x*C3
    ...
    memslots1 .= coeff2.*memslots1 .+ coeff3.*memslots2 .+ coeff4.*memslots3 .+ coeff5.*memslots4 .+ coeff6.*memslots6
