...
@inline function ps_m12(A)
    return ps_m12!(copy(A))
end
...
@inline function ps_m12!(A)
    ...
    value_one=ValueOne()
    # Computation order: A2 A3 A4 A5 A6 A7 P6 C5 P5 C4 P4 C3 P3 C2 P2 C1 P1 C0 P0
    ...
    # Computing P6 = x*I+x*A+x*A2+x*A3+x*A4+x*A5+x*A6+x*A7
    coeff1=7.117406731291439e-52
    coeff2=1.6552108677421951e-53
    coeff3=3.7618428812322616e-55
    coeff4=8.359650847182804e-57
    coeff5=1.817315401561479e-58
    coeff6=3.866628513960594e-60
    coeff7=8.055476070751236e-62
    coeff8=1.643974708316579e-63
    ...
    # Deallocating P6 in slot 8
    # Computing P5 = x*I+x*A+x*A2+x*A3+x*A4+x*A5+x*A6+x*C5
    ...
    memslots[1] .= coeff2.*memslots[1] .+ coeff3.*memslots[2] .+ coeff4.*memslots[3] .+ coeff5.*memslots[4] .+ coeff6.*memslots[5] .+ coeff7.*memslots[6] .+ coeff8.*memslots[8]
