...
@inline function exp_mono_m11(A)
    T=promote_type(eltype(A),Float64)
    A_copy=similar(A,T); A_copy .= A;
    return exp_mono_m11!(A_copy)
end
...
@inline function exp_mono_m11!(A)
    ...
    memslots12 = similar(A,T)
    # Assign precomputed nodes memslots 
    memslots1=A # overwrite A
    # Uniform scaling is exploited.
    # No matrix I explicitly allocated.
    value_one=ValueOne()
    # Computation order: B2 B3 B4 B5 B6 B7 B8 B9 B10 B11 B12 T2k14
    ...
    mul!(memslots12,memslots11,memslots1)
    # Computing T2k14 = x*I+x*A+x*B2+x*B3+x*B4+x*B5+x*B6+x*B7+x*B8+x*B9+x*B10+x*B11+x*B12
    coeff1=1.0
    coeff2=1.0
    coeff3=0.5
    ...
    memslots1 .= coeff2.*memslots1 .+ coeff3.*memslots2 .+ coeff4.*memslots3 .+ coeff5.*memslots4 .+ coeff6.*memslots5 .+ coeff7.*memslots6 .+ coeff8.*memslots7 .+ coeff9.*memslots8 .+ coeff10.*memslots9 .+ coeff11.*memslots10 .+ coeff12.*memslots11 .+ coeff13.*memslots12
    ...
    return memslots1 # Returning T2k14
end
