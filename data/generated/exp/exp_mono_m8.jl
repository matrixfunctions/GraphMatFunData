...
@inline function exp_mono_m8(A)
    return exp_mono_m8!(copy(A))
end
...
@inline function exp_mono_m8!(A)
    ...
    value_one=ValueOne()
    # Computation order: B2 B3 B4 B5 B6 B7 B8 B9 T2k11
    ...
    # Computing T2k11 = x*I+x*A+x*B2+x*B3+x*B4+x*B5+x*B6+x*B7+x*B8+x*B9
    coeff1=1.0
    coeff2=1.0
    coeff3=0.5
    ...
    memslots[1] .= coeff2.*memslots[1] .+ coeff3.*memslots[2] .+ coeff4.*memslots[3] .+ coeff5.*memslots[4] .+ coeff6.*memslots[5] .+ coeff7.*memslots[6] .+ coeff8.*memslots[7] .+ coeff9.*memslots[8] .+ coeff10.*memslots[9]
